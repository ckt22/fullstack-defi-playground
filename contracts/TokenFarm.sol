...
contract TokenFarm is Ownable {
    ...
    function tokenIsAllowed(address _token) public returns(bool) {
        for (uint256 allowedTokenIndex = 0; allowedTokenIndex < allowedTokens.length; allowedTokenIndex++) {
            if (allowedTokens[allowedTokenIndex] == _token) return true;
        }
        return false;
    }
    // stake
    ...
    function unstakeTokens(address _token) public {
        ...
        // if the user no longer holds any balance
        // remove them from the stakers array
    ...
    // get the token price and decimals from the price feed
